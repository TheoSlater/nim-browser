export function customizationGuide(displayName) {
    return `# Customizing ${displayName}
...
After the first build, use the watcher:

~~~bash
npm run dev
~~~

It restarts directly for profile CSS, runs mach build faster for packaged frontend changes, and runs a full build for config, patches, native code, WebIDL, or build-system files. Saves are debounced and builds never overlap.
...
export async function showBanner(window) {
  await window.gNotificationBox.appendNotification(
    "${displayName.toLowerCase().replaceAll(' ', '-')}-announcement",
    {
...
~~~js
const seenPref = "${displayName.toLowerCase().replaceAll(' ', '-')}.announcement.seen";
...
export function registerDashboardButton() {
  CustomizableUI.createWidget({
    id: "${displayName.toLowerCase().replaceAll(' ', '-')}-dashboard-button",
    type: "button",
    label: "Open dashboard",
    tooltiptext: "Open ${displayName} dashboard",
...
~~~css
#${displayName.toLowerCase().replaceAll(' ', '-')}-dashboard-button {
...
~~~css
:root {
  --${displayName.toLowerCase().replaceAll(' ', '-')}-accent: #7c3aed;
}
...
.tabbrowser-tab[selected] .tab-background {
  outline: 2px solid var(--${displayName.toLowerCase().replaceAll(' ', '-')}-accent);
}
...
~~~json
{
  "browser": {
    "name": "${displayName}",
    "binaryName": "${displayName.toLowerCase().replaceAll(' ', '-')}",
    "vendor": "Your Company",
    "applicationId": "com.yourcompany.${displayName.toLowerCase().replaceAll(' ', '')}"
  }
}
~~~
...
cp .geckokit/firefox/browser/base/content/browser-init.js \
  src/firefox/browser/base/content/browser-init.js
~~~
